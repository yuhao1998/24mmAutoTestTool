#!/system/bin/sh
# HalSelfCheck — light（soft-skip：缺依赖/不可用则 skip，不报 Fail）
...
MODULE_ID="light"
MODULE_NAME="灯光 HAL"
HAL_BINARY="/vendor/bin/hw/vendor.iauto.hardware.light@1.0-service"
PROCESS_PAT="light@1.0-service"
LSHAL_NEEDLE="vendor.iauto.hardware.light@1.0::ILight/default"
TOMBSTONE_KW="light"
...
OUT="${1:-${T2_SELFCHECK_OUT:-/data/local/tmp/t2_selfcheck/light}}"
...
soft_skip_all_l1() {
  ...
  run_l2_if_enabled "${HAL_ENABLE_BRIGHTNESS_WRITE_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/brightness_write_restore.sh" \
    "brightness_write_restore" "brightness_write_restore" "false"

  run_l2_if_enabled "${HAL_ENABLE_DISPLAY_MUTE_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/display_mute_restore.sh" \
    "display_mute_restore" "display_mute_restore" "false"

  run_l2_if_enabled "${HAL_ENABLE_DISPLAY_SWITCH_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/display_switch_restore.sh" \
    "display_switch_restore" "display_switch_restore" "false"

  run_l2_if_enabled "${HAL_ENABLE_INVALID_DISPLAYID_EXPECTED_FAIL:-false}" \
    "$SCRIPT_DIR/cases/invalid_displayid_expected_fail.sh" \
    "invalid_displayid_expected_fail" "invalid_displayid_expected_fail" "false"
  finish_and_exit
}
