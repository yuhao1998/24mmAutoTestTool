#!/system/bin/sh
# HalSelfCheck — ampservice（soft-skip：缺依赖/不可用则 skip，不报 Fail）
...
MODULE_ID="ampservice"
MODULE_NAME="功放 AMPService"
HAL_BINARY="/vendor/bin/AMPService"
PROCESS_PAT="AMPService"
LSHAL_NEEDLE="vendor.hsae.hardware.ampcontrol@1.0::IAmpControl/amp_control"
TOMBSTONE_KW="AMPService|ampcontrol"
BINARY_OPTIONAL="false"
MODE="hidl"
SOFT_IF_UNAVAILABLE="1"
ENV_DEPS="most"
SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
OUT="${1:-${T2_SELFCHECK_OUT:-/data/local/tmp/t2_selfcheck/ampservice}}"
...
soft_skip_all_l1() {
  ...
  run_l2_if_enabled "${HAL_ENABLE_MAIN_VOLUME_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/main_volume_restore.sh" \
    "main_volume_restore" "main_volume_restore" "false"
  ...
  run_l2_if_enabled "${HAL_ENABLE_SOURCE_STATE_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/source_state_restore.sh" \
    "source_state_restore" "source_state_restore" "false"

  run_l2_if_enabled "${HAL_ENABLE_TONE_PARAMS_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/tone_params_restore.sh" \
    "tone_params_restore" "tone_params_restore" "false"
  finish_and_exit
}
