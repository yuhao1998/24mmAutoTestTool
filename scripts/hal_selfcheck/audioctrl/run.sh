#!/system/bin/sh
# HalSelfCheck — audioctrl（soft-skip：缺依赖/不可用则 skip，不报 Fail）
# 源码：ssh://192.168.64.47:29418/Android/QCOM_SA8155_6155/platform/vendor/hsae/proprietary/hardware 分支 24MM_T2_dev
# SOFT_IF_UNAVAILABLE=0 ENV_DEPS=
# 无 CANoe/对手件硬依赖；L2 默认 skip

MODULE_ID="audioctrl"
MODULE_NAME="音频控制 HAL"
HAL_BINARY="/vendor/bin/hw/vendor.iauto.hardware.audioctrl@1.1-service"
PROCESS_PAT="audioctrl@1.1-service"
LSHAL_NEEDLE="vendor.iauto.hardware.audioctrl@1.1::IAudioCtrl/default"
TOMBSTONE_KW="audioctrl"
BINARY_OPTIONAL="false"
MODE="hidl"
SOFT_IF_UNAVAILABLE="0"
ENV_DEPS=""
SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
OUT="${1:-${T2_SELFCHECK_OUT:-/data/local/tmp/t2_selfcheck/audioctrl}}"
...
soft_skip_all_l1() {
  ...
  run_l2_if_enabled "${HAL_ENABLE_AMP_DIAG_START_STOP:-false}" \
    "$SCRIPT_DIR/cases/amp_diag_start_stop.sh" \
    "amp_diag_start_stop" "amp_diag_start_stop" "false"

  run_l2_if_enabled "${HAL_ENABLE_AUDIO_PARAMS_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/audio_params_restore.sh" \
    "audio_params_restore" "audio_params_restore" "false"

  run_l2_if_enabled "${HAL_ENABLE_CHANNEL_MIX_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/channel_mix_restore.sh" \
    "channel_mix_restore" "channel_mix_restore" "false"

  run_l2_if_enabled "${HAL_ENABLE_GROUP_VOLUME_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/group_volume_restore.sh" \
    "group_volume_restore" "group_volume_restore" "false"

  run_l2_if_enabled "${HAL_ENABLE_LISTENER_REGISTER_CLEANUP:-false}" \
    "$SCRIPT_DIR/cases/listener_register_cleanup.sh" \
    "listener_register_cleanup" "listener_register_cleanup" "false"

  run_l2_if_enabled "${HAL_ENABLE_MUTE_WRITE_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/mute_write_restore.sh" \
    "mute_write_restore" "mute_write_restore" "false"

  run_l2_if_enabled "${HAL_ENABLE_SOURCE_SWITCH_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/source_switch_restore.sh" \
    "source_switch_restore" "source_switch_restore" "false"

  run_l2_if_enabled "${HAL_ENABLE_TIMED_MUTE_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/timed_mute_restore.sh" \
    "timed_mute_restore" "timed_mute_restore" "false"
  finish_and_exit
}
