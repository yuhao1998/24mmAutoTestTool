#!/system/bin/sh
# HalSelfCheck — maintainhal（soft-skip：缺依赖/不可用则 skip，不报 Fail）
# 源码：ssh://192.168.64.47:29418/Android/QCOM_SA8155_6155/platform/vendor/hsae/proprietary/hardware 分支 24MM_T2_dev
# SOFT_IF_UNAVAILABLE=1 ENV_DEPS=most
# 无 CANoe/对手件硬依赖；L2 默认 skip

MODULE_ID="maintainhal"
MODULE_NAME="MaintainHal 维护诊断"
HAL_BINARY="/vendor/bin/hw/vendor.hsae.hardware.maintain@1.0-service"
PROCESS_PAT="maintain@1.0-service"
LSHAL_NEEDLE="vendor.hsae.hardware.maintain@1.0::IMaintainHal/default"
TOMBSTONE_KW="maintain"
BINARY_OPTIONAL="false"
MODE="hidl"
SOFT_IF_UNAVAILABLE="1"
ENV_DEPS="most"
SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
OUT="${1:-${T2_SELFCHECK_OUT:-/data/local/tmp/t2_selfcheck/maintainhal}}"
...
soft_skip_all_l1() {
  ...
  run_l2_if_enabled "${HAL_ENABLE_AMP_SPEAKER_CHECK_EXIT:-false}" \
    "$SCRIPT_DIR/cases/amp_speaker_check_exit.sh" \
    "amp_speaker_check_exit" "amp_speaker_check_exit" "false"

  run_l2_if_enabled "${HAL_ENABLE_CLEAR_HISTORY_BLOCKED:-false}" \
    "$SCRIPT_DIR/cases/clear_history_blocked.sh" \
    "clear_history_blocked" "clear_history_blocked" "false"

  run_l2_if_enabled "${HAL_ENABLE_ENTER_EXIT_DIAGNOSIS:-false}" \
    "$SCRIPT_DIR/cases/enter_exit_diagnosis.sh" \
    "enter_exit_diagnosis" "enter_exit_diagnosis" "false"

  run_l2_if_enabled "${HAL_ENABLE_SELF_DIAGNOSIS:-false}" \
    "$SCRIPT_DIR/cases/self_diagnosis.sh" \
    "self_diagnosis" "self_diagnosis" "false"

  run_l2_if_enabled "${HAL_ENABLE_VUP_BLOCKED:-false}" \
    "$SCRIPT_DIR/cases/vup_blocked.sh" \
    "vup_blocked" "vup_blocked" "false"
  finish_and_exit
}
