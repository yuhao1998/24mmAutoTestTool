#!/system/bin/sh
# HalSelfCheck — rse（soft-skip：缺依赖/不可用则 skip，不报 Fail）
...
MODULE_ID="rse"
MODULE_NAME="后排 RSE"
HAL_BINARY="/vendor/bin/hw/vendor.iauto.hardware.rse@1.0-service"
PROCESS_PAT="rse@1.0-service"
LSHAL_NEEDLE="vendor.iauto.hardware.rse@1.0::IRSE/default"
TOMBSTONE_KW="rse@1.0"
BINARY_OPTIONAL="false"
MODE="hidl"
SOFT_IF_UNAVAILABLE="1"
ENV_DEPS=""
SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
OUT="${1:-${T2_SELFCHECK_OUT:-/data/local/tmp/t2_selfcheck/rse}}"
...
soft_skip_all_l1() {
  ...
  run_l2_if_enabled "${HAL_ENABLE_LOCK_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/lock_restore.sh" \
    "lock_restore" "lock_restore" "false"

  run_l2_if_enabled "${HAL_ENABLE_MUTE_KEYBOARD_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/mute_keyboard_restore.sh" \
    "mute_keyboard_restore" "mute_keyboard_restore" "false"

  run_l2_if_enabled "${HAL_ENABLE_RSEINPUT_LISTENER_CLEANUP:-false}" \
    "$SCRIPT_DIR/cases/rseinput_listener_cleanup.sh" \
    "rseinput_listener_cleanup" "rseinput_listener_cleanup" "false"

  run_l2_if_enabled "${HAL_ENABLE_SOURCE_VIDEO_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/source_video_restore.sh" \
    "source_video_restore" "source_video_restore" "false"
  finish_and_exit
}
