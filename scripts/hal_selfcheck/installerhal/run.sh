#!/system/bin/sh
# HalSelfCheck — installerhal（soft-skip：缺依赖/不可用则 skip，不报 Fail）
...
MODULE_ID="installerhal"
MODULE_NAME="OTA installerhal"
HAL_BINARY="/vendor/bin/hw/vendor.iauto.hardware.installerhal@1.0-service"
PROCESS_PAT="installerhal@1.0-service"
LSHAL_NEEDLE="vendor.iauto.hardware.installerhal.common@1.0::IInstallerHal/default"
TOMBSTONE_KW="installerhal"
BINARY_OPTIONAL="false"
MODE="hidl"
SOFT_IF_UNAVAILABLE="0"
ENV_DEPS=""
SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
OUT="${1:-${T2_SELFCHECK_OUT:-/data/local/tmp/t2_selfcheck/installerhal}}"
...
soft_skip_all_l1() {
  ...
  run_l2_if_enabled "${HAL_ENABLE_INVENTORY_STATE_READONLY:-false}" \
    "$SCRIPT_DIR/cases/inventory_state_readonly.sh" \
    "inventory_state_readonly" "inventory_state_readonly" "false"

  run_l2_if_enabled "${HAL_ENABLE_OTA_INSTALL_ROLLBACK:-false}" \
    "$SCRIPT_DIR/cases/ota_install_rollback.sh" \
    "ota_install_rollback" "ota_install_rollback" "false"

  run_l2_if_enabled "${HAL_ENABLE_PREINSTALL_CONDITION_CHECK:-false}" \
    "$SCRIPT_DIR/cases/preinstall_condition_check.sh" \
    "preinstall_condition_check" "preinstall_condition_check" "false"

  run_l2_if_enabled "${HAL_ENABLE_SET_SLOT_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/set_slot_restore.sh" \
    "set_slot_restore" "set_slot_restore" "false"
  finish_and_exit
}
