#!/system/bin/sh
# HalSelfCheck — devmanager（soft-skip：缺依赖/不可用则 skip，不报 Fail）
...
MODULE_ID="devmanager"
MODULE_NAME="设备管理 HAL"
HAL_BINARY="/vendor/bin/hw/vendor.iauto.hardware.devmanager@1.1-service"
PROCESS_PAT="devmanager@1.1-service"
LSHAL_NEEDLE="vendor.iauto.hardware.devmanager@1.1::IDevManager/default"
TOMBSTONE_KW="devmanager"
BINARY_OPTIONAL="false"
MODE="hidl"
SOFT_IF_UNAVAILABLE="0"
ENV_DEPS=""
SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
OUT="${1:-${T2_SELFCHECK_OUT:-/data/local/tmp/t2_selfcheck/devmanager}}"
...
soft_skip_all_l1() {
  ...
  run_l2_if_enabled "${HAL_ENABLE_ATTRS_MAC_READONLY:-false}" \
    "$SCRIPT_DIR/cases/attrs_mac_readonly.sh" \
    "attrs_mac_readonly" "attrs_mac_readonly" "false"

  run_l2_if_enabled "${HAL_ENABLE_DANGEROUS_OPS_BLOCKED:-false}" \
    "$SCRIPT_DIR/cases/dangerous_ops_blocked.sh" \
    "dangerous_ops_blocked" "dangerous_ops_blocked" "false"

  run_l2_if_enabled "${HAL_ENABLE_RTC_PERSIST_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/rtc_persist_restore.sh" \
    "rtc_persist_restore" "rtc_persist_restore" "false"
  finish_and_exit
}
