#!/system/bin/sh
# HalSelfCheck — securitychip（soft-skip：缺依赖/不可用则 skip，不报 Fail）
...
MODULE_ID="securitychip"
MODULE_NAME="安全芯片 securityta100"
HAL_BINARY="/vendor/bin/hw/vendor.iauto.hardware.securitychip@1.0-service"
PROCESS_PAT="securitychip@1.0-service"
LSHAL_NEEDLE="vendor.iauto.hardware.securitychip@1.0::IHsmManager/default"
TOMBSTONE_KW="securitychip|ta100"
BINARY_OPTIONAL="false"
MODE="hidl"
SOFT_IF_UNAVAILABLE="1"
ENV_DEPS=""
SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
OUT="${1:-${T2_SELFCHECK_OUT:-/data/local/tmp/t2_selfcheck/securitychip}}"
...
soft_skip_all_l1() {
  ...
  run_l2_if_enabled "${HAL_ENABLE_CHIP_VERSION_READONLY:-false}" \
    "$SCRIPT_DIR/cases/chip_version_readonly.sh" \
    "chip_version_readonly" "chip_version_readonly" "false"

  run_l2_if_enabled "${HAL_ENABLE_KEY_UPDATE_BLOCKED:-false}" \
    "$SCRIPT_DIR/cases/key_update_blocked.sh" \
    "key_update_blocked" "key_update_blocked" "false"

  run_l2_if_enabled "${HAL_ENABLE_TA_UPGRADE_BLOCKED:-false}" \
    "$SCRIPT_DIR/cases/ta_upgrade_blocked.sh" \
    "ta_upgrade_blocked" "ta_upgrade_blocked" "false"
  finish_and_exit
}
