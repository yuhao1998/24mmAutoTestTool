#!/system/bin/sh
# HalSelfCheck — earlycarservice（soft-skip：缺依赖/不可用则 skip，不报 Fail）
# 源码：ssh://192.168.64.47:29418/Android/QCOM_SA8155_6155/platform/vendor/hsae/proprietary/hardware 分支 24MM_T2_dev
# SOFT_IF_UNAVAILABLE=1 ENV_DEPS=mcu
# 无 CANoe/对手件硬依赖；L2 默认 skip

MODULE_ID="earlycarservice"
MODULE_NAME="EarlyCarService 串口"
HAL_BINARY="/vendor/bin/hw/vendor.hsae.hardware.earlycarservice.serial@1.0-service"
PROCESS_PAT="earlycarservice.serial@1.0-service"
LSHAL_NEEDLE="vendor.hsae.hardware.earlycarservice.serial@1.0::ISerial/default"
TOMBSTONE_KW="earlycarservice"
BINARY_OPTIONAL="false"
MODE="hidl"
SOFT_IF_UNAVAILABLE="1"
ENV_DEPS="mcu"
SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
OUT="${1:-${T2_SELFCHECK_OUT:-/data/local/tmp/t2_selfcheck/earlycarservice}}"
...
soft_skip_all_l1() {
  ...
  run_l2_if_enabled "${HAL_ENABLE_CALLBACK_CLEANUP:-false}" \
    "$SCRIPT_DIR/cases/callback_cleanup.sh" \
    "callback_cleanup" "callback_cleanup" "false"

  run_l2_if_enabled "${HAL_ENABLE_HANDSHAKE_HEARTBEAT_SMOKE:-false}" \
    "$SCRIPT_DIR/cases/handshake_heartbeat_smoke.sh" \
    "handshake_heartbeat_smoke" "handshake_heartbeat_smoke" "false"

  run_l2_if_enabled "${HAL_ENABLE_SEND_DATA_SMOKE:-false}" \
    "$SCRIPT_DIR/cases/send_data_smoke.sh" \
    "send_data_smoke" "send_data_smoke" "false"
  finish_and_exit
}
