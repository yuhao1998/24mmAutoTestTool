#!/system/bin/sh
# HalSelfCheck — drinfo（soft-skip：缺依赖/不可用则 skip，不报 Fail）
...
MODULE_ID="drinfo"
MODULE_NAME="航位 DRInfo"
HAL_BINARY="/vendor/bin/hw/vendor.iauto.hardware.drinfo@1.0-service"
PROCESS_PAT="drinfo@1.0-service"
LSHAL_NEEDLE="vendor.iauto.hardware.drinfo@1.0::IDRInfoController/default"
TOMBSTONE_KW="drinfo"
...
OUT="${1:-${T2_SELFCHECK_OUT:-/data/local/tmp/t2_selfcheck/drinfo}}"
...
soft_skip_all_l1() {
  ...
  run_l2_if_enabled "${HAL_ENABLE_CALLBACK_REGISTER_CLEANUP:-false}" \
    "$SCRIPT_DIR/cases/callback_register_cleanup.sh" \
    "callback_register_cleanup" "callback_register_cleanup" "false"

  run_l2_if_enabled "${HAL_ENABLE_SENSOR_STREAM_READONLY:-false}" \
    "$SCRIPT_DIR/cases/sensor_stream_readonly.sh" \
    "sensor_stream_readonly" "sensor_stream_readonly" "false"
  finish_and_exit
}
