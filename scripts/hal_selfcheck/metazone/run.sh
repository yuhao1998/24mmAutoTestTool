#!/system/bin/sh
# HalSelfCheck — metazone（soft-skip：缺依赖/不可用则 skip，不报 Fail）
...
MODULE_ID="metazone"
MODULE_NAME="MetaZone 分区存储"
HAL_BINARY="/vendor/bin/hw/vendor.hsae.hardware.metazone@1.0-service"
PROCESS_PAT="metazone@1.0-service"
LSHAL_NEEDLE="vendor.hsae.hardware.metazone@1.0::IMetazone/default"
TOMBSTONE_KW="metazone"
BINARY_OPTIONAL="true"
MODE="hidl"
SOFT_IF_UNAVAILABLE="1"
ENV_DEPS=""
SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
OUT="${1:-${T2_SELFCHECK_OUT:-/data/local/tmp/t2_selfcheck/metazone}}"
...
soft_skip_all_l1() {
  ...
  run_l2_if_enabled "${HAL_ENABLE_BINARY_WRITE_RESTORE_FLUSH:-false}" \
    "$SCRIPT_DIR/cases/binary_write_restore_flush.sh" \
    "binary_write_restore_flush" "binary_write_restore_flush" "false"

  run_l2_if_enabled "${HAL_ENABLE_DWORD_WRITE_RESTORE_FLUSH:-false}" \
    "$SCRIPT_DIR/cases/dword_write_restore_flush.sh" \
    "dword_write_restore_flush" "dword_write_restore_flush" "false"

  run_l2_if_enabled "${HAL_ENABLE_FLUSH_ONLY:-false}" \
    "$SCRIPT_DIR/cases/flush_only.sh" \
    "flush_only" "flush_only" "false"

  run_l2_if_enabled "${HAL_ENABLE_SPEC_RESERVED_BLOCKED:-false}" \
    "$SCRIPT_DIR/cases/spec_reserved_blocked.sh" \
    "spec_reserved_blocked" "spec_reserved_blocked" "false"
  finish_and_exit
}
