#!/system/bin/sh
# HalSelfCheck — mostslave（soft-skip：缺依赖/不可用则 skip，不报 Fail）
...
MODULE_ID="mostslave"
MODULE_NAME="MOST 从节点"
HAL_BINARY="/vendor/bin/hw/vendor.hsae.hardware.mostslave@1.0-service"
PROCESS_PAT="mostslave@1.0-service"
LSHAL_NEEDLE="vendor.hsae.hardware.mostslave@1.0::IMostSlaveHal/default"
TOMBSTONE_KW="mostslave"
BINARY_OPTIONAL="false"
MODE="hidl"
SOFT_IF_UNAVAILABLE="1"
ENV_DEPS="most"
SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
OUT="${1:-${T2_SELFCHECK_OUT:-/data/local/tmp/t2_selfcheck/mostslave}}"
...
soft_skip_all_l1() {
  ...
  run_l2_if_enabled "${HAL_ENABLE_FBLOCK_CALLBACK_CLEANUP:-false}" \
    "$SCRIPT_DIR/cases/fblock_callback_cleanup.sh" \
    "fblock_callback_cleanup" "fblock_callback_cleanup" "false"

  run_l2_if_enabled "${HAL_ENABLE_FBLOCK_GET_INTERFACE:-false}" \
    "$SCRIPT_DIR/cases/fblock_get_interface.sh" \
    "fblock_get_interface" "fblock_get_interface" "false"

  run_l2_if_enabled "${HAL_ENABLE_START_STOP_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/start_stop_restore.sh" \
    "start_stop_restore" "start_stop_restore" "false"
  finish_and_exit
}
