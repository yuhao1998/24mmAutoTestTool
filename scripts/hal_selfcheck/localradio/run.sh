#!/system/bin/sh
# HalSelfCheck — localradio（soft-skip：缺依赖/不可用则 skip，不报 Fail）
...
MODULE_ID="localradio"
MODULE_NAME="收音 localradio"
HAL_BINARY="/vendor/bin/hw/vendor.iauto.hardware.localradio@1.0-service"
PROCESS_PAT="localradio@1.0-service"
LSHAL_NEEDLE="vendor.iauto.hardware.localradio@1.0::ILocalRadio/default"
TOMBSTONE_KW="localradio"
BINARY_OPTIONAL="false"
MODE="hidl"
SOFT_IF_UNAVAILABLE="1"
ENV_DEPS=""
SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
OUT="${1:-${T2_SELFCHECK_OUT:-/data/local/tmp/t2_selfcheck/localradio}}"
...
soft_skip_all_l1() {
  ...
  run_l2_if_enabled "${HAL_ENABLE_PRESET_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/preset_restore.sh" \
    "preset_restore" "preset_restore" "false"

  run_l2_if_enabled "${HAL_ENABLE_RADIO_MUTE_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/radio_mute_restore.sh" \
    "radio_mute_restore" "radio_mute_restore" "false"

  run_l2_if_enabled "${HAL_ENABLE_SEEK_SCAN_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/seek_scan_restore.sh" \
    "seek_scan_restore" "seek_scan_restore" "false"

  run_l2_if_enabled "${HAL_ENABLE_TUNE_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/tune_restore.sh" \
    "tune_restore" "tune_restore" "false"
  finish_and_exit
}
