#!/system/bin/sh
# HalSelfCheck — diag（soft-skip：缺依赖/不可用则 skip，不报 Fail）
...
MODULE_ID="diag"
MODULE_NAME="诊断 HAL"
HAL_BINARY="/vendor/bin/hw/vendor.iauto.hardware.diag@1.0-service"
PROCESS_PAT="diag@1.0-service"
LSHAL_NEEDLE="vendor.iauto.hardware.diag@1.0::IDiag/default"
TOMBSTONE_KW="diag"
...
OUT="${1:-${T2_SELFCHECK_OUT:-/data/local/tmp/t2_selfcheck/diag}}"
...
soft_skip_all_l1() {
  ...
  run_l2_if_enabled "${HAL_ENABLE_CAN_SEND_READONLY_SESSION:-false}" \
    "$SCRIPT_DIR/cases/can_send_readonly_session.sh" \
    "can_send_readonly_session" "can_send_readonly_session" "false"

  run_l2_if_enabled "${HAL_ENABLE_CLEAR_DTC_BLOCKED:-false}" \
    "$SCRIPT_DIR/cases/clear_dtc_blocked.sh" \
    "clear_dtc_blocked" "clear_dtc_blocked" "false"

  run_l2_if_enabled "${HAL_ENABLE_LISTENER_CLEANUP:-false}" \
    "$SCRIPT_DIR/cases/listener_cleanup.sh" \
    "listener_cleanup" "listener_cleanup" "false"

  run_l2_if_enabled "${HAL_ENABLE_SELFCHECK_START_STOP:-false}" \
    "$SCRIPT_DIR/cases/selfcheck_start_stop.sh" \
    "selfcheck_start_stop" "selfcheck_start_stop" "false"

  run_l2_if_enabled "${HAL_ENABLE_SPEAKER_CHECK_QUIT:-false}" \
    "$SCRIPT_DIR/cases/speaker_check_quit.sh" \
    "speaker_check_quit" "speaker_check_quit" "false"
  finish_and_exit
}
