#!/system/bin/sh
# HalSelfCheck — anc（soft-skip：缺依赖/不可用则 skip，不报 Fail）
...
MODULE_ID="anc"
MODULE_NAME="ANC ASC 查询"
HAL_BINARY="/vendor/bin/hw/vendor.hsae.hardware.anc@1.0-service"
PROCESS_PAT="anc@1.0-service"
LSHAL_NEEDLE="vendor.hsae.hardware.anc@1.0::IANC/default"
TOMBSTONE_KW="anc@1.0"
BINARY_OPTIONAL="false"
MODE="hidl"
SOFT_IF_UNAVAILABLE="1"
ENV_DEPS="most"
SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
OUT="${1:-${T2_SELFCHECK_OUT:-/data/local/tmp/t2_selfcheck/anc}}"
...
soft_skip_all_l1() {
  ...
  run_l2_if_enabled "${HAL_ENABLE_GET_ASCINFO_READONLY:-false}" \
    "$SCRIPT_DIR/cases/get_ascinfo_readonly.sh" \
    "get_ascinfo_readonly" "get_ascinfo_readonly" "false"
