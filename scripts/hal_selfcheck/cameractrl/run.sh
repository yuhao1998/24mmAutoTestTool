#!/system/bin/sh
# HalSelfCheck — cameractrl（soft-skip：缺依赖/不可用则 skip，不报 Fail）
# 源码：ssh://192.168.64.47:29418/Android/QCOM_SA8155_6155/platform/vendor/hsae/proprietary/hardware 分支 24MM_T2_dev
# SOFT_IF_UNAVAILABLE=1 ENV_DEPS=camera
# 无 CANoe/对手件硬依赖；L2 默认 skip

MODULE_ID="cameractrl"
MODULE_NAME="相机控制 HAL"
HAL_BINARY="/vendor/bin/hw/vendor.iauto.hardware.cameractrl@1.0-service"
PROCESS_PAT="cameractrl@1.0-service"
LSHAL_NEEDLE="vendor.iauto.hardware.cameractrl@1.0::ICameraCtrl/default"
TOMBSTONE_KW="cameractrl"
BINARY_OPTIONAL="false"
MODE="hidl"
SOFT_IF_UNAVAILABLE="1"
ENV_DEPS="camera"
SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
OUT="${1:-${T2_SELFCHECK_OUT:-/data/local/tmp/t2_selfcheck/cameractrl}}"
...
soft_skip_all_l1() {
  ...
  run_l2_if_enabled "${HAL_ENABLE_DIAG_STATUS_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/diag_status_restore.sh" \
    "diag_status_restore" "diag_status_restore" "false"
  ...
  run_l2_if_enabled "${HAL_ENABLE_OPEN_CLOSE_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/open_close_restore.sh" \
    "open_close_restore" "open_close_restore" "false"

  run_l2_if_enabled "${HAL_ENABLE_READY_FLAGS_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/ready_flags_restore.sh" \
    "ready_flags_restore" "ready_flags_restore" "false"
  finish_and_exit
}
