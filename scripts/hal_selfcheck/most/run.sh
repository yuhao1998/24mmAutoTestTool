#!/system/bin/sh
# HalSelfCheck — most（soft-skip：缺依赖/不可用则 skip，不报 Fail）
# 源码：ssh://192.168.64.47:29418/Android/QCOM_SA8155_6155/platform/vendor/hsae/proprietary/hardware 分支 24MM_T2_dev
# SOFT_IF_UNAVAILABLE=1 ENV_DEPS=most_hw
# 无 CANoe/对手件硬依赖；L2 默认 skip

MODULE_ID="most"
MODULE_NAME="Most 环网 HAL"
HAL_BINARY="/vendor/bin/hw/vendor.hsae.hardware.most@1.0-service"
PROCESS_PAT="most@1.0-service"
LSHAL_NEEDLE="vendor.hsae.hardware.most@1.0::IMostHal/default"
TOMBSTONE_KW="most@1.0"
BINARY_OPTIONAL="false"
MODE="hidl"
SOFT_IF_UNAVAILABLE="1"
ENV_DEPS="most_hw"
SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
OUT="${1:-${T2_SELFCHECK_OUT:-/data/local/tmp/t2_selfcheck/most}}"
...
soft_skip_all_l1() {
  ...
  run_l2_if_enabled "${HAL_ENABLE_AMP_SETGET_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/amp_setget_restore.sh" \
    "amp_setget_restore" "amp_setget_restore" "false"

  run_l2_if_enabled "${HAL_ENABLE_ANC_SETTING_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/anc_setting_restore.sh" \
    "anc_setting_restore" "anc_setting_restore" "false"

  run_l2_if_enabled "${HAL_ENABLE_RSE_SUBSET_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/rse_subset_restore.sh" \
    "rse_subset_restore" "rse_subset_restore" "false"
