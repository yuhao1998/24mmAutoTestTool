#!/system/bin/sh
# HalSelfCheck — input（soft-skip：缺依赖/不可用则 skip，不报 Fail）
...
MODULE_ID="input"
MODULE_NAME="输入 HAL"
HAL_BINARY="/vendor/bin/hw/vendor.iauto.hardware.input@1.0-service"
PROCESS_PAT="input@1.0-service"
LSHAL_NEEDLE="vendor.iauto.hardware.input@1.0::IInput/default"
TOMBSTONE_KW="input@1.0"
BINARY_OPTIONAL="false"
MODE="hidl"
SOFT_IF_UNAVAILABLE="0"
ENV_DEPS=""
SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
OUT="${1:-${T2_SELFCHECK_OUT:-/data/local/tmp/t2_selfcheck/input}}"
...
soft_skip_all_l1() {
  ...
  run_l2_if_enabled "${HAL_ENABLE_TOUCH_SENSITIVITY_RESTORE:-false}" \
    "$SCRIPT_DIR/cases/touch_sensitivity_restore.sh" \
    "touch_sensitivity_restore" "touch_sensitivity_restore" "false"
  finish_and_exit
}
