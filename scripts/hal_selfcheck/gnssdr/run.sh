#!/system/bin/sh
# HalSelfCheck — gnssdr（soft-skip：缺依赖/不可用则 skip，不报 Fail）
...
MODULE_ID="gnssdr"
MODULE_NAME="Gnssdr"
HAL_BINARY="/vendor/bin/hw/vendor.iauto.hardware.gnssdr@1.0-service"
PROCESS_PAT="gnssdr@1.0-service"
LSHAL_NEEDLE="vendor.iauto.hardware.gnssdr@1.0::IGnssdrd/default"
TOMBSTONE_KW="gnssdr"
...
OUT="${1:-${T2_SELFCHECK_OUT:-/data/local/tmp/t2_selfcheck/gnssdr}}"
...
soft_skip_all_l1() {
  ...
  run_l2_if_enabled "${HAL_ENABLE_NMEA_CALLBACK_READONLY:-false}" \
    "$SCRIPT_DIR/cases/nmea_callback_readonly.sh" \
    "nmea_callback_readonly" "nmea_callback_readonly" "false"
  finish_and_exit
}
