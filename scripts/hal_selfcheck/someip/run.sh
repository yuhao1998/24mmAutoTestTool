#!/system/bin/sh
# HalSelfCheck — someip（soft-skip：缺依赖/不可用则 skip，不报 Fail）
...
MODULE_ID="someip"
MODULE_NAME="SOME/IP 守护进程"
HAL_BINARY="/vendor/bin/isomeipd_ics"
PROCESS_PAT="isomeipd_ics"
LSHAL_NEEDLE=""
TOMBSTONE_KW="isomeipd"
BINARY_OPTIONAL="false"
MODE="daemon"
SOFT_IF_UNAVAILABLE="1"
ENV_DEPS=""
SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
OUT="${1:-${T2_SELFCHECK_OUT:-/data/local/tmp/t2_selfcheck/someip}}"
...
soft_skip_all_l1() {
  ...
  run_l2_if_enabled "${HAL_ENABLE_DAEMON_LIFECYCLE_SMOKE:-false}" \
    "$SCRIPT_DIR/cases/daemon_lifecycle_smoke.sh" \
    "daemon_lifecycle_smoke" "daemon_lifecycle_smoke" "false"

  run_l2_if_enabled "${HAL_ENABLE_E2E_PROTOCOL_LAB:-false}" \
    "$SCRIPT_DIR/cases/e2e_protocol_lab.sh" \
    "e2e_protocol_lab" "e2e_protocol_lab" "false"

  run_l2_if_enabled "${HAL_ENABLE_GTEST_CLIENT_SERVER:-false}" \
    "$SCRIPT_DIR/cases/gtest_client_server.sh" \
    "gtest_client_server" "gtest_client_server" "false"
  finish_and_exit
}
